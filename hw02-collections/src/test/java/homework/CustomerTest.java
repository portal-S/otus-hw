...
class CustomerTest {
    ...
    //раз мы рассчитываем на id как показатель уникальности то нужно поддерживать контракт
    // что у нас нет возможности как в тесте создавать 2 разных сущности с одним id
    // т.е. в таком случае тест customerAsKeyTest был бы не корректен
    //да и в целом иметь изменяемую сущность как ключ плохая идея
    @Test
